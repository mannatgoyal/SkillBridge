    ...
    return (
        <div className="group relative flex-shrink-0 w-[250px] md:w-[300px] aspect-video bg-surface rounded-md overflow-hidden cursor-pointer transition-transform duration-300 hover:scale-110 hover:z-10 border border-transparent hover:border-border">
            ...
            <div className="absolute inset-0 p-4 flex flex-col justify-end bg-gradient-to-t from-black/90 via-black/50 to-transparent">
                <h3 className="text-base md:text-lg font-bold text-white group-hover:text-primary transition-colors leading-tight">
                    {role.title}
                </h3>
                ...
                <div className="h-0 opacity-0 group-hover:h-auto group-hover:opacity-100 transition-all duration-300 overflow-hidden">
                    <div className="pt-2 flex items-center gap-2 text-[10px] md:text-xs text-green-400 font-semibold">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                        {role.requiredSkills.slice(0, 3).map((skill, idx) => (
                            <span key={idx} className="text-[10px] px-1.5 py-0.5 bg-white/10 rounded text-gray-200 border border-white/10">
                                {skill.name}
                            </span>
                        ))}
                    </div>
